                            ...
                            <div class="card-content collapse show">
                                <div class="card-body card-dashboard">
                                    ...
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        <tbody>
											@foreach($paiedOrders as $index => $order)
                                            <tr>
                                                <td>#{{ $index + 1 }}</td>
                                                <td>{{ fmDate( $order->created_at, "Y-m-d H:i:s", "d/m/Y" ) }}</td>
                                                
                                                <td>
                                                    <button type="button" class="btn btn-sm btn-outline-success round" onclick="window.location.href='{{ route('workshop.order.detail', $order->id) }}'">{{ $order->currentStatus() }}</button>
                                                </td>
                                            </tr>
											@endforeach
										</tbody>
							...
							<div class="card-content collapse show">
                                <div class="card-body card-dashboard">
                                    ...
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        <tbody>
											@foreach($proceedOrders as $index => $order)
                                            <tr>
                                                <td>#{{ $index + 1 }}</td>
                                                <td>{{ fmDate( $order->created_at, "Y-m-d H:i:s", "d/m/Y" ) }}</td>
